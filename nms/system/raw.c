...
static __inline__ int zhz_icmp_filter(struct sock *sk, struct sk_buff *skb)
{
	/*printk(KERN_ALERT "skb->h.icmph->un.echo.id == %d"
		"sk->tp_pinfo.tp_raw4.filter.data == %d\n",
		skb->h.icmph->un.echo.id,
		sk->tp_pinfo.tp_raw4.filter.data);
	*/
	if (sk->zhz_icmp_filter)
		if (ICMP_ECHOREPLY == skb->h.icmph->type &&
			skb->h.icmph->un.echo.id == 
				sk->tp_pinfo.tp_raw4.filter.data)
			return 0;
		else {
			//printk(KERN_ALERT "%d.%d.%d.%d -- pid (%d) "
			//	"type (%d)\n",
...
struct sock *raw_v4_input(struct sk_buff *skb, struct iphdr *iph, int hash)
{
	...
	while(sk != NULL) {
		struct sock *sknext = __raw_v4_lookup(sk->next, iph->protocol,
						      ...
						      skb->dev->ifindex);
		if (
		    (iph->protocol != IPPROTO_ICMP && !sk->zhz_icmp_filter)
////////////////////////////////////////////////////////////////////
		    //
		    || (sk->zhz_icmp_filter && !zhz_icmp_filter(sk, skb))  
		    //
////////////////////////////////////////////////////////////////////
		    || (!sk->zhz_icmp_filter && !icmp_filter(sk, skb))
		) {
			  ...
			  char *optval, int optlen)
{
	...
	//printk(KERN_ALERT "raw_setsockopt\n");
	switch (optname) {
	case ICMP_FILTER:
		//printk(KERN_ALERT "raw_setsockopt old\n");
		if (sk->num != IPPROTO_ICMP)
			return -EOPNOTSUPP;
		sk->zhz_icmp_filter = 0;
		return raw_seticmpfilter(sk, optval, optlen);
////////////////////////////////////////////////////////////////////
//
	case ZHZ_ICMP_FILTER:
		//printk(KERN_ALERT "raw_setsockopt zhz\n");
		if (sk->num != IPPROTO_ICMP)
			return -EOPNOTSUPP;
		sk->zhz_icmp_filter = 1;
		if (optlen > sizeof(struct icmp_filter))
			optlen = sizeof(struct icmp_filter);
		if (copy_from_user(&sk->tp_pinfo.tp_raw4.filter.data, optval, optlen))
			return -EFAULT;
		//printk(KERN_ALERT "sk->tp_pinfo.tp_raw4.filter.data == %d\n",
		//	sk->tp_pinfo.tp_raw4.filter.data);
		return 0;
